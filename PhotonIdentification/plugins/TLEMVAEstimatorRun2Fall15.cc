...
// rhoForEleToken = consumes<double>(LeptonIsoHelper::getEleRhoTag(sampleType, setup));
  ...
  if( (int)(weightFileNames.size()) != NUM_CAT )
    throw cms::Exception("MVA config failure: ")
      << "wrong number of weightfiles. Expected " << NUM_CAT << " but got " << weightFileNames.size() << std::endl;
...
std::vector<float> TLEMVAEstimatorRun2Fall15::fillMVAVariables(const edm::Ptr<reco::Candidate>& particle, const edm::Event& iEvent) const {
  ...
  float PFChargedHadIso   = pat_pho->chargedHadronIso();
  float PFPhotonIso       = pat_pho->photonIso();
  float PFNeutralHadIso   = pat_pho->neutralHadronIso();

  float ele_HZZ_iso = (PFChargedHadIso + std::max((float) 0., PFNeutralHadIso + PFPhotonIso + (float) 0. * (float) rhoForEle))/particle->pt(); 
